...
Begin VB.Form FrmTimesheet 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFlexGrid1 
      ...
      FormatString    =   $"frmTimesheet.frx":00C5
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   -1  'True
      MergeCells      =   0
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   0
      OleObjectBlob   =   "frmTimesheet.frx":01A4
      Top             =   0
   End
   Begin VSFlex8Ctl.VSFlexGrid VsFlex 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"frmTimesheet.frx":03D8
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   -1  'True
      MergeCells      =   0
   ...
   Begin VSFlex8Ctl.VSFlexGrid fg 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"frmTimesheet.frx":04B7
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   -1  'True
      MergeCells      =   0
...
Cboid = "08:00|08:30|09:00|09:30|10:00|10:30|11:00|11:30|12:00|13:00|13:30|14:00|14:30|15:00|15:30|16:00|16:30|17:00|17:30|18:00|18:30|19:00|19:30|20:00|20:30|21:00|21:30|22:00|22:30|23:00|23:30|00:00|00:30|01:00|01:30|02:00|02:30|03:00|03:30|04:00|04:30|05:00|05:30|06:00|06:30|07:00|07:30"
    ...
    StrSQL = "Select IDtimesheet As Do,IDtimesheet,JamAwal As [Jam Awal],JamAkhir AS [Jam Akhir],Kd_divisi AS Divisi,NoProject As Project,Phase,Keterangan,Tanggal,Masuk,StatusPM From tbltimesheet Where Tanggal = '" & Format(DTPicker1, "yyyy/MM/dd") & "' And NIP = '" & StrNIPUser & "'  AND Status='Actual' Order By IDTimesheet Asc"
    ...
    For Lrow = 1 To .Rows - 2
        ...
        .TextMatrix(Lrow, 1) = ""
        If Trim(.TextMatrix(Lrow, 8)) = "Lembur" Then
           For i = 3 To .Cols - 2
               .Row = Lrow
               .Col = i
           ...
           Next
        End If
        If .TextMatrix(Lrow, 11) <> "" Then
       If .TextMatrix(Lrow, 11) = 1 Then
            For i = 1 To .Cols - 2
                .Row = Lrow
                .Col = i
...
End Sub
Sub AddPhase(Nama As String)
Dim Cboid, cboid1 As String
    If Rscek.State = adStateOpen Then Rscek.Close
    
    StrSQL = "select *  from tblPhase Where kd_divisi='" & Nama & "' order by ID Asc"
    Rscek.Open StrSQL, CN, adOpenStatic
    cboid1 = " "
    Do Until Rscek.EOF
      Cboid = "|" & Rscek("Kode_Phase") & vbTab & Rscek("Keterangan")
      cboid1 = cboid1 + Cboid
      Rscek.MoveNext
    Loop
        VsFlex.ColComboList(7) = cboid1
...
If Row = 1 Then
    If Col = 3 Or Col = 4 Then
   'Untuk Ngcek Absen And .TextMatrix(Row, 8) = "Timesheet"
...
Else
    If Col = 3 Or Col = 4 Then
        ...
        End If
            ...
            If .TextMatrix(.Row, 8) = "Lembur" And .TextMatrix(.Row, 4) <> "" Then
        ...
        For i = 1 To .Rows - 2
           If (Jam1 = Format(.TextMatrix(i, 3), "HH:mm") Or Jam1 < Format(.TextMatrix(i, 4), "HH:mm")) And i < Row And .TextMatrix(.Row, 8) = "Timesheet" Then
              MsgBox "Invalid Time", vbCritical
               FillList
              Exit Sub
           End If
...
On Error Resume Next
 ...
 If .TextMatrix(.Row - 1, 3) = "" Then .Editable = flexEDNone: Exit Sub
 If .TextMatrix(.Row - 1, 6) = "" Then .Editable = flexEDNone: Exit Sub
    If .Col = 8 Or .CellBackColor = &HE0E0E0 Then
        .Editable = flexEDNone
        .TextMatrix(.Row, 1) = ""
        Exit Sub
    Else
...
If Col = 8 Then KeyAscii = 0: Exit Sub
If KeyAscii = 39 Then KeyAscii = 96
...
If .TextMatrix(Row, 2) = "" Then
      '-----Ambil ID -------------------
        ...
        StrKodeID = NewID
    '---------------------------------
    StrSQL = "Insert Into tbltimesheet(IDtimesheet,NIP,kd_divisi,Status,NoProject,Tanggal,JamAwal,JamAkhir,Phase,keterangan,Masuk,Keluar,last_update,last_user,StatusPM,StatusDivisi,Hari,TotalKerja,ProjectUmum) Values "
    StrSQL = StrSQL & "('" & StrKodeID & "','" & StrNIPUser & "','" & .TextMatrix(Row, 5) & "','Actual','" & .TextMatrix(Row, 6) & "','" & Format(DTPicker1, "MM/dd/yyyy") & "','" & .TextMatrix(Row, 3) & "','" & .TextMatrix(Row, 4) & "','" & .TextMatrix(Row, 7) & "','" & .TextMatrix(Row, 8) & "','" & LabelAbsen & "','" & Label5 & "','" & ServerTime & "','" & StrUser & "',0,0,'" & StatusHari & "','" & TotalJamPUmum & "', '" & ProjectUmum & "')"
...
Else
    ...
    StrSQL = "Update tbltimesheet Set Hari = '" & StatusHari & "',kd_divisi='" & .TextMatrix(Row, 5) & "',Status='Actual' ,Tanggal='" & Format(DTPicker1, "MM/dd/yyyy") & "',JamAwal='" & .TextMatrix(Row, 3) & "',JamAkhir='" & .TextMatrix(Row, 4) & "',NoProject = '" & .TextMatrix(Row, 6) & "',Phase='" & .TextMatrix(Row, 7) & "',keterangan='" & .TextMatrix(Row, 8) & "',Masuk='" & LabelAbsen & "',last_update='" & ServerTime & "',last_user='" & StrNIPUser & "',TotalKerja = '" & TotalJamPUmum & "', ProjectUmum= '" & ProjectUmum & "' Where IDtimesheet='" & .TextMatrix(Row, 2) & "' "
    PerintahExecute (StrSQL)
...
'        .TextMatrix(PosRow, 2) = Format(VSFg.TextMatrix(i, 2), "dd/MM/yyyy")
        ...
        Project = VSFg.TextMatrix(i, 3) & "/" & VSFg.TextMatrix(i, 11)
