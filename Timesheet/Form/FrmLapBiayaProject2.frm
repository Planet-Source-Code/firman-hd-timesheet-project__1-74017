...
Begin VB.Form FrmLapBiayaProject2 
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VSFlex8Ctl.VSFlexGrid cboFlex 
         ...
         FormatString    =   $"FrmLapBiayaProject2.frx":0000
         ScrollTrack     =   0   'False
         ScrollBars      =   0
         ScrollTips      =   0   'False
         MergeCells      =   0
      ...
      Begin VSFlex8Ctl.VSFlexGrid CboKaryawan 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   -1  'True
         FormatString    =   $"FrmLapBiayaProject2.frx":0029
         ScrollTrack     =   0   'False
         ScrollBars      =   0
         ScrollTips      =   0   'False
         MergeCells      =   0
      ...
      Begin VSFlex8Ctl.VSFlexGrid CboDivisi 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   -1  'True
         FormatString    =   $"FrmLapBiayaProject2.frx":0052
         ScrollTrack     =   0   'False
         ScrollBars      =   0
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   120
      OleObjectBlob   =   "FrmLapBiayaProject2.frx":007B
      Top             =   0
   End
   Begin VSFlex8Ctl.VSFlexGrid fg 
      ...
      FormatString    =   $"FrmLapBiayaProject2.frx":02AF
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   -1  'True
      MergeCells      =   0
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSGaji 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"FrmLapBiayaProject2.frx":038E
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   -1  'True
      MergeCells      =   0
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFg 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"FrmLapBiayaProject2.frx":046D
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   -1  'True
      MergeCells      =   0
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFlexGrid1 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"FrmLapBiayaProject2.frx":054C
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   -1  'True
      MergeCells      =   0
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSJoin 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"FrmLapBiayaProject2.frx":062B
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   -1  'True
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "FrmLapBiayaProject2"
...
.TextMatrix(0, 4) = "NIP"
.TextMatrix(0, 5) = "Nama"
.TextMatrix(0, 6) = "Total Kerja"
.TextMatrix(0, 7) = "Biaya Gaji"
.TextMatrix(0, 8) = "Jam Lembur"
.TextMatrix(0, 9) = "Biaya Lembur"
.TextMatrix(0, 10) = "U.Makan"
...
'If Trim(CboDivisi.Text) = "" Then
'   MsgBox "Silahkan Pilih Divisi Terlebih Dahulu", vbCritical
'   cboFlex.SetFocus
'   Exit Sub
'End If
Command1.Enabled = False
With VSFlexGrid1
    ...
    StrSQL = "SELECT tbltimesheet.IDtimesheet,tbltimesheet.Tanggal,tbltimesheet.JamAwal As [Jam Awal],tbltimesheet.JamAkhir AS [Jam Akhir],tbltimesheet.Status,tbltimesheet.NoProject As Project,tbltimesheet.Keterangan,tbltimesheet.Tanggal,Absensi.Masuk,tbltimesheet.NIP,tbltimesheet.StatusDivisi, karyawan.Nama,tbltimesheet.StatusPM,Absensi.Keluar,Divisi.NM_DIV, Project.Nama AS NamaProject,tbltimesheet.Hari,tbltimesheet.TotalKerja,tbltimesheet.ProjectUmum"
    StrSQL = StrSQL & " FROM tbltimesheet INNER JOIN karyawan ON tbltimesheet.NIP = karyawan.NIP INNER JOIN Divisi ON karyawan.Kd_Divisi = Divisi.KD_DIV INNER JOIN Absensi ON tbltimesheet.NIP = Absensi.NIP AND tbltimesheet.Tanggal = Absensi.Tgl INNER JOIN Project ON tbltimesheet.NoProject = Project.Kode"
    StrSQL = StrSQL & " Where tbltimesheet.Tanggal Between '" & Format(DTPicker1, "MM/dd/yyyy") & "' And '" & Format(DTPicker2, "MM/dd/yyyy") & "'  And tbltimesheet.Status ='Actual' And StatusPM = 1 And StatusDivisi = 1 And Project.Status = 'Terpakai'"
...
Dim tPersen As Double
    ...
    For Lrow = 1 To .Rows - 2
        Tjam = Tjam + CDbl(.TextMatrix(Lrow, 13))
        Gaji = Gaji + CDbl(.TextMatrix(Lrow, 7))
        .TextMatrix(Lrow, 14) = Round(.TextMatrix(Lrow, 13) / .TextMatrix(Lrow, 11) * 100, 2)
        tPersen = tPersen + Round(CDbl(.TextMatrix(Lrow, 14)), 2)
'          .TextMatrix(Lrow, 6) = .TextMatrix(Lrow, 6) & "/" & .TextMatrix(Lrow, 14) & "%"
        .Row = Lrow
        .Col = 6
        .CellAlignment = flexAlignCenterCenter
        tlembur = tlembur + CDbl(.TextMatrix(Lrow, 8))
        lembur = CDbl(lembur) + CDbl(.TextMatrix(Lrow, 9))
        .TextMatrix(Lrow, 0) = Lrow
    Next
    .TextMatrix(.Rows - 1, 13) = Tjam
    .TextMatrix(.Rows - 1, 14) = tPersen
    .TextMatrix(.Rows - 1, 6) = Tjam & ":" & tPersen
    .TextMatrix(.Rows - 1, 7) = Gaji
    .TextMatrix(.Rows - 1, 8) = tlembur
    .TextMatrix(.Rows - 1, 9) = lembur
End With
...
With fg
    ...
    If Trim(.TextMatrix(FRow, 1)) = Divisi And Trim(.TextMatrix(FRow, 4)) = NIP And Trim(.TextMatrix(FRow, 2)) = Project Then
           StatusProject = True
           HRow = FRow
        End If
    Next
        ...
        End If
            ...
            .TextMatrix(HRow, 3) = VSJoin.TextMatrix(Row, 8)
            .TextMatrix(HRow, 4) = VSJoin.TextMatrix(Row, 3)
            .TextMatrix(HRow, 5) = VSJoin.TextMatrix(Row, 4)
            ...
            If Trim(Keterangan) = "Timesheet" Then
                ...
                .TextMatrix(HRow, 7) = CCur(.TextMatrix(HRow, 7)) + VSJoin.TextMatrix(Row, 15)
                .TextMatrix(HRow, 13) = CCur(.TextMatrix(HRow, 13)) + CCur(VSJoin.TextMatrix(Row, 5))
'                .TextMatrix(HRow, 7) = CCur(.TextMatrix(HRow, 7)) + VSJoin.TextMatrix(Row, 15)
            Else
                .TextMatrix(HRow, 8) = CDbl(.TextMatrix(HRow, 8)) + CDbl(VSJoin.TextMatrix(Row, 10))
                .TextMatrix(HRow, 9) = CCur(.TextMatrix(HRow, 9)) + VSJoin.TextMatrix(Row, 16)
            End If
End With
